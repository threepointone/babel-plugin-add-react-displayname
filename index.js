    ...
    visitor: {
      ...
      FunctionDeclaration: function(path, state) {
        if (
          ...
          (path.node.id &&
            path.node.id.name &&
            ((path.node.id.name.charCodeAt(0) >= 65 &&
              path.node.id.name.charCodeAt(0) <= 90) ||
              path.node.id.name.slice(0, 3) === "use" ||
...
function shouldSetDisplayNameForFuncExpr(path, knownComponents) {
  ...
  if (id) {
    if (id.name) {
      if (isKnownComponent(id.name, knownComponents)) {
        return true;
      }
      ...
      t.callExpression(t.identifier("__register__"), [
        ...
        t.stringLiteral(
          displayName +
            ...
            require("crypto")
              .createHash("md5")
              .update(path.hub.file.parserOpts.sourceFileName)
              .digest("hex")
        )
      ])
    );
